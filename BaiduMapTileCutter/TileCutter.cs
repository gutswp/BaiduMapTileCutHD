...
namespace BaiduMapTileCutter
{
    public class TileCutter
    {
        ...
        private int calcTotalTilesByZoom(int zoom)
        {
            ...
            Point bottomLeftTileCoords = bottomLeftPixel.Divide(256);//网格坐标
            ...
            Point upperRightTileCoords = upperRightPixel.Divide(256);//网格坐标
        ...
        {
            int halfWidth = (int)Math.Round((double)imgFile.Width / 2);//图片半宽
            int halfHeight = (int)Math.Round((double)imgFile.Height / 2);//图片半高
            ...
            Point mcPoint = Projection.fromLatLngToPoint(center);
            // 中心点在整个世界中的像素坐标
            Point pxPoint = mcPoint.Multiply(Math.Pow(2, zoom - 18));//说明百度18级是标准的墨卡托坐标？？
            pxPoint.Round();
            ...
            // 图片左下角的像素坐标和网格编号
            Point bottomLeftPixel = new Point(pxPoint.X - halfWidth, pxPoint.Y - halfHeight);//像素坐标
            bottomLeftPixel.Round();

            Point bottomLeftTileCoords = bottomLeftPixel.Divide(256);//网格编号
            bottomLeftTileCoords.Floor();
            // 图片右上角的像素坐标和网格编号
            ...
            for (int i = 0; i < totalCols; i++)
            {
                for (int j = 0; j < totalRows; j++)
                {
                    ...
                    Point eachTileCoords = new Point(bottomLeftTileCoords.X + i, bottomLeftTileCoords.Y + j);//每个瓦片图的左下角坐标

                    //当前瓦片图左上角相对于源图的坐标，源图左上角为远点，向右、向下，x、y值延伸
                    ...
                    #region 修正，修改当前切片的左上角相对于图片远点左上角的相对偏移，原点为源图的左上角
                    ...
                    #region 更新成新的命名方式.tiles/12/x/y.png,对标前端的离线地图格式
                    string dirpath = outputPath + "/tiles/" + zoom.ToString()
                    ...
                    imgFileName = outputPath + "/tiles/" + zoom.ToString()
                        + "//" + eachTileCoords.X.ToString() + "//" + eachTileCoords.Y.ToString() + ".jpg";
                    #endregion
